  ...
  async execute(dto: CreateCheckpointDTO): Promise<CheckpointResponseDTO> {
    if (!dto.unitId || !dto.trackingId || !dto.status) {
      throw new ValidationError(
        'Missing required fields: unitId, trackingId, and status are required',
      );
    }

    if (
      !Object.values(CheckpointStatus).includes(dto.status as CheckpointStatus)
    ) {
    ...
    if (unit) {
      unit.updateStatus(dto.status as CheckpointStatus, savedCheckpoint);
